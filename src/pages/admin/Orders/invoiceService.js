        ...
        <div class="container">
          ...
          <div class="info-section">
            <div class="info-box">
              ...
              ${order.vendors && order.vendors.length > 0 ? order.vendors.map(vendor => `
                <div style="margin-bottom:16px;">
                  <p><strong>${vendor.name || 'Satıcı Bilgisi Yok'}</strong></p>
                  ${vendor.tax_id ? `<p>Vergi No: ${vendor.tax_id}</p>` : ''}
                  ${vendor.phone ? `<p>Tel: ${vendor.phone}</p>` : ''}
                  ${vendor.email ? `<p>${vendor.email}</p>` : ''}
                </div>
              `).join('') : '<p>Satıcı bilgisi yok</p>'}
            </div>
            <div class="info-box">
              ...
              <p style="margin-top:12px"><strong>Ödeme:</strong> ${order.paymentMethod || 'Belirtilmemiş'}</p>
            </div>
          </div>
          ...
          <div class="footer">
            ...
            <p>Ticaret.com Admin Paneli tarafından oluşturulmuştur.</p>
          </div>
        </div>
